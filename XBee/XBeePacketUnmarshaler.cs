    ...
    public class XBeePacketUnmarshaler
    {
        ...
        private static Dictionary<XBeeAPICommandId, Type> CreateFramesMap()
        {
            var map = new Dictionary<XBeeAPICommandId, Type>
            {
                ...
                {XBeeAPICommandId.TX_STATUS_RESPONSE, typeof (TxStatusResponse)}
            };
        ...
        {
            XBeeFrame frame;
            var length = (ushort) (packetData[0] << 8 | packetData[1]);

            if (length == 0 || length > 0xFFFF)
                throw new XBeeFrameException(String.Format("Invalid Frame Length {0}.", length));

            if (length != packetData.Length - 3)
                throw new XBeeFrameException(String.Format("Invalid Frame Length - Expecting {0}, received {1}.", length,
                    packetData.Length - 3));
        ...
        public static void RegisterFrameHandler(XBeeAPICommandId commandId, Type typeHandler)
        {
            ...
            {
                Logger.Info(String.Format("Overriding Frame Handler: {0} with {1} for API Id: 0x{2:x2}",
                    FramesMap[commandId].Name, typeHandler.Name, (byte) commandId));
                FramesMap[commandId] = typeHandler;
            }
            else
            {
                Logger.Info(String.Format("Adding Frame Handler: {0} for API Id: 0x{1:x2}", typeHandler.Name,
                    (byte) commandId));
